...
sys.path.insert(0, os.path.join(root_dir, '..', 'libraries', 'node', 'tools', 'configure.d'))
import nodedownload
...
def configure_node(o):
  ...
  if flavor in ('solaris', 'mac', 'linux', 'freebsd'):
    ...
    o['variables']['uv_parent_path'] = '../libraries/node/deps/uv/'
...
output['variables']['win_subsystem'] = options.subsystem
if make_global_settings:
  output['make_global_settings'] = make_global_settings

pprint.pprint(output, indent=2)

write('../libraries/node/config.gypi', do_not_edit +
...
gyp_args = [sys.executable, 'tools/gyp_tint', '--no-parallel']
